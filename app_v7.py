...
def baue_heatmap(konsens_df, sonne=None):
    ...
    for i, tag in enumerate(tage_list):
        for j in stunden:
            ...
            custom[i][j] = [
                r['ampel'], r['sicherheit'],
                f"{r['temp_median']:.0f}" if r['temp_median'] is not None else '–',
                f"{r['temp_spread']:.0f}",
                f"{r['wind_median']:.0f}" if r['wind_median'] is not None else '–',
                f"{r['feuchte_median']:.0f}" if r['feuchte_median'] is not None else '–',
                f"{r['ns_median']:.1f}", f"{r['ns_wahrscheinlichkeit']:.0f}",
                int(r['n_laeufe']), r['gruende'],
                (f"{r['taupunkt']:.0f}" if r.get('taupunkt') is not None
                 and not pd.isna(r.get('taupunkt')) else '–'),
                (f"{r['boeen']:.0f}" if r.get('boeen') is not None
                 and not pd.isna(r.get('boeen')) else '–'),
                (himmelsrichtung(r.get('windrichtung'))
                 if r.get('windrichtung') is not None else '–'),
            ]
            if r['sicherheit'] == 'gering':
...
st.set_page_config(page_title="EnsembleWetter", page_icon="◭", layout="wide")


st.markdown("""
<style>
  .block-container {padding-top: 2.2rem; max-width: 1320px;}
  h1 {font-weight: 600; letter-spacing: -0.6px; margin-bottom: 0.15rem;}
  ...
  div[data-testid="stMetricLabel"] {font-size: 0.8rem; opacity: 0.72;}
  .ew-section {font-size: 1.08rem; font-weight: 600; letter-spacing: -0.2px;
               margin: 1.9rem 0 0.15rem 0;}
  .ew-sub {font-size: 0.85rem; opacity: 0.68; margin-bottom: 0.55rem;
           line-height: 1.45;}
  .ew-note {font-size: 0.8rem; opacity: 0.62; line-height: 1.5;
            margin-top: 0.35rem;}
  .ew-legend {display: flex; flex-wrap: wrap; gap: 0.55rem 1.1rem;
              align-items: center; font-size: 0.8rem; opacity: 0.85;
              margin: 0.5rem 0 0.2rem 0;}
  .ew-chip {display: inline-flex; align-items: center; gap: 0.4rem;}
  .ew-dot {width: 13px; height: 13px; border-radius: 3px; display: inline-block;}
  .ew-headline {font-size: 1.02rem; font-weight: 600; margin-bottom: 0.15rem;}
</style>
""", unsafe_allow_html=True)

st.title("EnsembleWetter")
st.caption("Präzisionsprognose für Landwirtschaft und Alpinraum — "
...
def stufen_legende():
    chips = ''.join(
        ...
        for s in [4, 3, 2, 1, 0])
    chips += ('<span class="ew-chip"><span style="font-weight:600">×</span>'
...
def parameter_editor(fall_name, basis):
    ...
    with st.expander("Richtwerte anpassen", expanded=False):
...
if ort_eingabe:
    ...
    if not treffer:
        st.sidebar.error(f"Standort '{ort_eingabe}' nicht gefunden.")
    elif len(treffer) == 1:
        gewaehlter_ort = treffer[0]
    else:
        with st.sidebar:
            labels = [t['label'] for t in treffer]
            wahl = st.selectbox("Mehrere Treffer — bitte wählen", labels)
            gewaehlter_ort = treffer[labels.index(wahl)]

if los and gewaehlter_ort:
    bar = st.progress(0.0, text="Analyse wird vorbereitet")
...
if 'konsens' in st.session_state:
    ...
    abschnitt("Eignungsmatrix",
              ...
              "Ein Feld anklicken, um die Detailwerte zu sehen.")
    stufen_legende()
    ...
    def windy_karte(overlay, hinweis, schluessel):
    ...
    st.info("Standort und Tätigkeit links auswählen, anschließend die "
            "Analyse starten.")
