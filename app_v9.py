...
# Letzter Ort aus URL-Parameter (gesetzt beim letzten Analysieren)
params = st.query_params
ort_default = params.get('ew_ort', 'Wieselburg')

# Favoriten aus session_state
if 'ew_favoriten' not in st.session_state:
    st.session_state['ew_favoriten'] = []

e1, e2 = st.columns([1, 1])
with e1:
    ort_eingabe = st.text_input("Standort", value=ort_default,
                                placeholder="Ort eingeben, z. B. Leoben")
    favs = st.session_state['ew_favoriten']
    if favs:
        fav_labels = ['\u2014 Favorit ausw\u00e4hlen \u2014'] + favs
        fav_wahl = st.selectbox("Favoriten", fav_labels,
                                label_visibility="collapsed",
                                key='fav_dropdown')
        if fav_wahl and fav_wahl != '\u2014 Favorit ausw\u00e4hlen \u2014':
...
# Favoriten-Button
_, fav_col = st.columns([3, 1])
with fav_col:
    if ort_eingabe and st.button("\u2605 Favorit speichern",
        ...
        st.success(f"\u201e{ort_eingabe}\u201c als Favorit gespeichert.")
...
if los and ort_eingabe:
    st.query_params['ew_ort'] = ort_eingabe
