...
def auswertung(anfrage: AuswertungAnfrage):
    ...
    # Zuerst direkt über die Kategorie, sonst über alle Kategorien suchen.
    # finde_fall() liefert ein Tupel (kategorie, parameter) — nicht nur
    # die Parameter.
    fall = ANWENDUNGSFAELLE.get(anfrage.kategorie, {}).get(anfrage.fall)
    if fall is None:
        _kat, fall = finde_fall(anfrage.fall)
    if fall is None:
        raise HTTPException(
            status_code=404,
