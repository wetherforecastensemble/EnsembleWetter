...
<style>
  ...
  .ew-warn {background:#fffaeb; border-left:3px solid var(--ew-amber);
             ...
             margin-top:0.4rem;}
  .ew-guete-quelle {background:#f2f9fb; border:1px solid #cfe6ec;
                    border-left:4px solid #22B5C2; border-radius:0 10px 10px 0;
                    padding:0.7rem 0.95rem; margin:0.2rem 0 0.9rem 0;
                    font-size:0.815rem; line-height:1.6; color:#17505c;}
  .ew-guete-quelle b {color:#0D223D;}
...
if 'konsens' in st.session_state:
    ...
    with st.expander("Prognosegüte — wie gut lagen die Modelle zuletzt?"):
        ...
        if not _guete:
            ...
            _inca = _guete.get('quelle') == 'inca'
            _womit = ('den <b>gemessenen Werten</b> der GeoSphere Austria'
                      if _inca else 'dem tatsächlichen Verlauf')
            st.markdown(
                ...
                f'und vergleichen sie mit {_womit}. So sehen Sie, wie weit im '
                f'Voraus Sie sich hier auf die Prognose verlassen können — und '
                f'ab wann es Glückssache wird.'
                f'</div>', unsafe_allow_html=True)
            if _inca:
                st.markdown(
                    '<div class="ew-guete-quelle">'
                    '<b>Referenz: INCA-Analyse der GeoSphere Austria.</b> '
                    'INCA verrechnet die Messungen des österreichischen '
                    'Stationsnetzes mit Wetterradar, Satellitendaten und '
                    'einem Geländemodell zu einem Raster von einem Kilometer '
                    'Kantenlänge — stündlich, für ganz Österreich. Verglichen '
                    'wird also mit tatsächlich gemessenem Wetter an genau '
                    'Ihrem Standort, nicht mit einer Wetterstation, die '
                    'irgendwo in der Umgebung steht, und nicht mit einem '
                    'anderen Modell.'
                    '</div>', unsafe_allow_html=True)
            ...
            for _v in sorted(_guete['vorlauf']):
                ...
                if _bias is not None and abs(_bias) >= 1.0:
                    ...
                    _zusatz = ('Gemessen gegen die INCA-Analyse ist das ein '
                               'belastbarer Befund. ' if _inca else
                               'Da hier gegen ein Modell verglichen wird, ist '
                               'der Befund mit Vorbehalt zu lesen. ')
                    st.markdown(
                        ...
                        f'{_zusatz}Rechnen Sie bei Frostgefahr entsprechend '
                        f'nach.</div>', unsafe_allow_html=True)
            ...
            if _inca:
                st.caption(
                    ...
                    f"{_guete['zeitraum_tage']} Tage, verglichen gegen die "
                    f"INCA-Analyse (1 km, stündlich) der GeoSphere Austria, "
                    f"bereitgestellt unter Creative Commons Attribution 4.0. "
                    f"INCA stützt sich auf Messungen, ist aber selbst eine "
                    f"Analyse: In Lagen mit wenigen Stationen — hohen Tälern, "
                    f"abgelegenen Gebieten — wird interpoliert, und beim "
                    f"Niederschlag können einzelne Schauer zwischen den "
                    f"Radarpixeln durchrutschen.")
            else:
                st.caption(
                    f"Datenbasis: {_n_stunden} Stunden der vergangenen "
                    f"{_guete['zeitraum_tage']} Tage. Für diesen Standort war "
                    f"keine Messanalyse verfügbar — er liegt außerhalb "
                    f"Österreichs oder die Schnittstelle antwortete nicht. "
                    f"Als Vergleichsgröße dient daher der aktuelle Modelllauf "
                    f"über den vergangenen Zeitraum. Fehler, die alle Läufe "
                    f"gleichermaßen betreffen, bleiben dabei unsichtbar; "
                    f"aussagekräftig ist vor allem, wie schnell die "
                    f"Genauigkeit mit der Vorlaufzeit nachlässt.")
    ...
    st.markdown(
        ...
        f'Alpinraum.<br>'
        f'<b>Wetterdaten:</b> <a href="https://open-meteo.com/" '
        f'target="_blank" rel="noopener">Open-Meteo</a>, lizenziert unter '
        f'<a href="https://creativecommons.org/licenses/by/4.0/" '
        f'target="_blank" rel="noopener">CC BY 4.0</a>; für diese Anwendung '
        f'aufbereitet und verändert dargestellt. Zugrunde liegende Modelle: '
        f'ECMWF IFS, DWD ICON, NOAA GFS, Météo-France ARPEGE/AROME, '
        f'GeoSphere AROME (Österreich), MeteoSchweiz ICON-CH, '
        f'Environment Canada GEM.<br>'
        f'<b>Ortssuche:</b> '
        f'<a href="https://www.openstreetmap.org/copyright" target="_blank" '
        f'rel="noopener">© OpenStreetMap-Mitwirkende</a>, abgefragt über '
        f'Nominatim; Daten unter '
        f'<a href="https://opendatacommons.org/licenses/odbl/" '
        f'target="_blank" rel="noopener">ODbL</a>.<br>'
        f'<b>Karten:</b> <a href="https://www.ventusky.com/" target="_blank" '
        f'rel="noopener">Ventusky</a> (InMeteo s.r.o.) auf Grundlage von '
        f'ICON, ECMWF und GFS, eingebunden über die offizielle '
        f'Einbettungsfunktion.<br>'
        f'<b>Prüfung der Prognosegüte:</b> INCA-Analysedaten der '
        f'<a href="https://www.geosphere.at" target="_blank" '
        f'rel="noopener">GeoSphere Austria</a> '
        f'(<a href="https://data.hub.geosphere.at/dataset/inca-v1-1h-1km" '
        f'target="_blank" rel="noopener">Datensatz</a>), lizenziert unter '
        f'<a href="https://creativecommons.org/licenses/by/4.0/" '
        f'target="_blank" rel="noopener">CC BY 4.0</a>.<br>'
        f'<b>Schrift:</b> Montserrat und Raleway unter '
        f'<a href="https://openfontlicense.org/" target="_blank" '
        f'rel="noopener">SIL Open Font License</a>.<br><br>'
