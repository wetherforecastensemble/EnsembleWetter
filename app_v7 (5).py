...
# Jede Stufe trägt neben der Farbe ein eigenes Symbol. Rot-Grün-Schwäche
# betrifft rund acht Prozent der Männer — und damit einen erheblichen Teil
# der Zielgruppe. Eine reine Farbkodierung wäre für diese Nutzer wertlos.
STUFEN = {
    4: {'name': 'sehr gut',   'farbe': '#2E8B57', 'kurz': 'sehr gut',
        'zeichen': '++', 'symbol': '\u25cf\u25cf', 'schrift': '#ffffff'},
    3: {'name': 'gut',        'farbe': '#66BB6A', 'kurz': 'gut',
        'zeichen': '+',  'symbol': '\u25cf',       'schrift': '#0D223D'},
    2: {'name': 'bedingt',    'farbe': '#FFC107', 'kurz': 'bedingt',
        'zeichen': 'o',  'symbol': '\u25cb',       'schrift': '#4a3800'},
...
def baue_heatmap(konsens_df, sonne=None):
    ...
    # Zweite Kodierung neben der Farbe, damit die Matrix auch bei
    # Farbsehschwäche und im Schwarzweißdruck lesbar bleibt
    zeichen = [['']*24 for _ in range(len(tage_list))]
    schriftfarben = [['#0D223D']*24 for _ in range(len(tage_list))]
    ...
    for i, tag in enumerate(tage_list):
        for j in stunden:
            ...
            r = zelle.iloc[0]
            z[i][j] = r['ampel_int']
            _st = STUFEN.get(int(r['ampel_int']), STUFEN[2])
            zeichen[i][j] = _st['symbol']
            schriftfarben[i][j] = _st['schrift']
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        text=zeichen, texttemplate='%{text}',
        textfont=dict(size=11, family='Raleway, sans-serif'),
        hovertemplate=(
            '<b>%{y}  %{x}:00</b><br>'
...
<style>
  ...
  .ew-dot {width: 20px; height: 16px; border-radius: 3px;
...
def stufen_legende():
    chips = ''.join(
        ...
        f'style="background:{STUFEN[s]["farbe"]};color:{STUFEN[s]["schrift"]}">'
        f'{STUFEN[s]["symbol"]}</span>{STUFEN[s]["name"]}</span>'
        for s in [4, 3, 2, 1, 0])
...
if 'konsens' in st.session_state:
    ...
    abschnitt("Eignungsmatrix",
              ...
              "nach Maschenweite und Vorlaufzeit. Ein Feld antippen, um die "
    ...
    with st.expander("Prognosegüte — wie gut lagen die Modelle zuletzt?"):
        ...
        if not _guete:
            ...
            st.caption(
                f"Verglichen werden die Vorhersagen der vergangenen "
                f"{_guete['zeitraum_tage']} Tage für diesen Standort mit dem "
                f"tatsächlichen Verlauf laut aktuellem Modelllauf. Das ist "
                f"die beste frei verfügbare Näherung, aber keine "
                f"Stationsmessung: Fehler, die auch die Analyse betreffen, "
                f"bleiben unsichtbar. Die Zahlen zeigen daher vor allem, "
                f"wie schnell die Treffsicherheit mit der Vorlaufzeit "
                f"abnimmt.")
            _zeilen = []
            for _v in sorted(_guete['vorlauf']):
                _e = _guete['vorlauf'][_v]
                _zeilen.append({
                    'Vorlauf': f'{_v} Tag{"e" if _v > 1 else ""}',
                    'Temperaturfehler': (f"{_e['temp_mae']:.1f} °C"
                                         if 'temp_mae' in _e else '–'),
                    'Systematische Abweichung': (
                        f"{_e['temp_bias']:+.1f} °C"
                        if 'temp_bias' in _e else '–'),
                    'Regen richtig erkannt': (
                        f"{_e['ns_entdeckung']:.0f} %"
                        if 'ns_entdeckung' in _e else '–'),
                    'davon Fehlalarme': (
                        f"{100 - _e['ns_praezision']:.0f} %"
                        if 'ns_praezision' in _e else '–'),
                })
            if _zeilen:
                st.dataframe(pd.DataFrame(_zeilen), hide_index=True,
                             use_container_width=True)
            st.caption(
                "Temperaturfehler: mittlere Abweichung ohne Vorzeichen. "
                "Systematische Abweichung: bleibt sie deutlich von null "
                "entfernt, rechnet das Modell an diesem Ort dauerhaft zu "
                "warm oder zu kalt — bei ausgeprägten Tallagen ein "
                "häufiges Muster. Regen richtig erkannt: Anteil der "
                "tatsächlichen Regenstunden, die vorhergesagt waren.")
        if hoehe is not None:
